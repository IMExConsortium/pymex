...
class RecordBuilder():
    ...
    def __init__(self,cvpath, debug=False):
        ...
        #also a weird file, but presumably those template JSONS in the other direction.
        srcpath = os.path.join( self.pdir,'jmif-source.jmif' )
        self.template["source"] = json.load( open( srcpath, 'r' ) )["source"]

        #default is null (at least with test case)
        if len(cvpath) == 0:
            print("jmif used")
    ...
    #input: txt file (short)
    #change root structure to interpret what to do with stroichiometry
    #should hold a reference for what to do when rebuilding it
    #output: root dictionary datastructure
    #create function that takes in xml xpath and inserts object into datastructure.
