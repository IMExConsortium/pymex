...
class Record( pymex.xmlrecord.XmlRecord ):
    ...
    def __init__( self, root=None, mode='dev', debug=False, mirror_xml=None, mirror_json=None ):

        myDir = os.path.dirname( os.path.realpath(__file__))
        self.config = { "eg001": {"IN": os.path.join( myDir, "upmatchParse001.json"),
                                  "OUT": os.path.join( myDir, "upmatchXml001.json" ) }
                       }
        ...
        self._mirror= { "xml": '/mirror/interpro/records/xml/unimatch',
                        "json": '/mirror/interpro/records/json/unimatch'}
    ...
    def getlocal( self, eg, format="xml" ):

        rec = None

        if self._debug:        
            print("getlocal: ", format, eg)
    ...
    def getRecord( self, eg, debug=False, delay=1 ):
        ...
        rec = self.getlocal( eg, format="json" )
        
        if rec is None:
            rec = self.getlocal( eg, format="xml" )
