    ...
    def mifElement(self, nsmap, ver, template, celem, cdata, cdef ):
        """Returns a list of DOM elements to be added to the parent and/or decorates  
           ...
           Arguments:
             ...
             - celem    - current dom element
             - cdata    - current data
             - cdef     - current format definition               
        """
        if "name" in cdef and cdef["name"] == "attributeX":
            print(cdef["name"])
        #else:
        #    print("N/A")
        ...
        retLst = []
        if "wrap" in cdef:
            # add wrapper when needed            
            wrapName = cdef["wrap"]
            wrapElem = ET.Element(self.toNsString(nsmap) + wrapName)        
        else:
            wrapElem = None
            
        if "value" not in cdef: # empty wrapper defined
            #print("no value")
            ...
            for wtDef in wrappedTypes: # go over wrapper content
                #print("generating wrapper contents " + str(wtDef) )
                
                if wtDef["value"] in cdata:   # check if data present
                    
                    wElemData = cdata[wtDef["value"]]
                    if "attribute" == wtDef["value"] :
                        print(" wrapped data " + str(wElemData) )
                        ...
                        for wtp in template[wtDef["type"]]:
                            ...
                            if wclLst is not None:
                                for wcd in wclLst:  
                                    chldElem.append(wcd)
                #else:
                #    print("node data for " + str(wtDef) )
            if not empty:                                  
                return [wrapElem]
            else:
                return None

        # if got here, definition "value" present     
            
        if cdef["value"] =="$UID": #  generate next id
            self.UID += 1
            elemData = str(self.UID)   # data is autogenerated  
        else:
            if cdef["value"] in cdata: # data from the record 
                elemData = cdata[ cdef["value"] ]
            else:                      # no data present 

                # LS: returns empty list if no data present.
                #        
                #  it would be better not to return here so that  
                #  there's a chance to call pos process at the end 
                #  of the method
                
                return [] # no data present: return empty list                   

        # if got here, data for the current element is in elemData
        ...
        if isinstance( elemData, str):  # record field: text
            ...
            if elemName.startswith('@'):  
                if elemName == "@TEXT" : # add as text of parent                
                    celem.text = str(elemData)
                else:                    # add as attribute of parent
        ...
        if isinstance( elemData, dict): # record field: just one complex element 
        ...
        # LS: if here, either wrapElem or retLst contain dom elements
        #     corresponding to data value
        #
        # posprocess called here should add, if needed, additional
        # dom elements to the list    

        if "post" in cdef: 
            print("post: " + cdef["post"])
            if wrapElem is not None:
                # call post-method(wrapElem,...)
                # this should add, if needed,  element(s) to wrapElem 
                pass
            else:
                # call post-method(retLst,...)
                # this should add, if needed,  element(s) to retLst 
                pass
